    ...
    public function zcf_NormalFieldAjaxAction() {
        ...
        switch ($_REQUEST['doaction']) {
            case "SaveFormSettings":
                ...
                $shortcodedata['assignto'] = isset($_REQUEST['assignedto']);
                $shortcodedata['errormesg'] = isset($_REQUEST['errormessage']);
                $shortcodedata['successmesg'] = isset($_REQUEST['successmessage']);
                $shortcodedata['duplicate_handling'] = isset($_REQUEST['duplicate_handling']);
                $shortcodedata['assignmentrule_ID'] = $_REQUEST['assignmentrule_ID'];
                $shortcodedata['assignmentrule_enable'] = $_REQUEST['assignmentrule_enable'];
                if (sanitize_text_field($_REQUEST['enableurlredirection']) == "true") {
                    $shortcodedata['isredirection'] = 1;
                } else {
                    $shortcodedata['isredirection'] = 0;
                }
                $shortcodedata['urlredirection'] = isset($_REQUEST['redirecturl']);
...
class zcf_CapturingClassAjax {
    ...
    function zcf_CaptureFormFieldsList($globalvariables) {
        ...
        if($shortcode_name ==''){
            $queryresult = "select crmformsshortcodename from zcf_contactformrelation where thirdpartyformid =".$post['formnumber'];
            $zcf_contactformrelation = $wpdb->get_results($queryresult);
